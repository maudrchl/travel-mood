...
class DraggableImage {
    ...
    initEvents() {
        ...
        this.onPointerDown = () => {
            ...
            this.outerScale.current = 0.6;
            const radius = (this.boundingRect.width + this.boundingRect.height)/1.7;
            for (let i = 0; i <= this.trailsTotal - 1; ++i) {
                ...
                this.trailsTranslation[i].current.x = this.draggie.position.x - parseFloat(this.trailsTranslation[i].random.x);
                this.trailsTranslation[i].current.y = this.draggie.position.y - parseFloat(this.trailsTranslation[i].random.y);
                this.trailsScale[i].current = 0.8;
                this.trailsRotation[i].current = getRandomFloat(-20,20);
            }
    ...
    layout() {
        ...
        for (let i = 0; i <= this.trailsTotal - 1; ++i) {
            const blurVal = clamp(map(draggingDistance, 0, 500, 0, 20), 0, 20);
